  ...
  for (const model of FALLBACK_MODELS) {
    try {
      ...
      if (model !== FALLBACK_MODELS[0]) {
        console.log(
          `[gemini] primary model failed, succeeded with fallback: ${model}`
        )
      }
...
export async function getPopupDescription (
  placeName,
  district,
  language = 'en'
) {
  const langForce =
    language === 'bn'
      ...
      : 'IMPORTANT: Respond in English only, even if place names are in Bengali.'
  const prompt =
    language === 'bn'
...
export async function getRouteGuide (
  placeNames,
  districts,
  totalDistance,
  totalDuration,
